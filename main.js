...
window.hantarDataKeGoogleSheets = async function() {
    ...
    try {
        ...
        console.error("[Sheets Engine Crash]: Gagal mengepos data.", error);
...
window.paparLeaderboardVisual = async function() {
    ...
    try {
        ...
        data.forEach((row, index) => {
            ...
            // Tukar format masa ke Minit:Saat untuk paparan kemas
            ...
            htmlContent += `
                <div class="flex justify-between items-center border-b border-cyan-500/10 py-2 text-sm text-white hover:bg-cyan-950/20 px-2 rounded transition-all">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-bold text-cyan-200 uppercase text-xs md:text-sm">${row.nama}</p>
                            <p class="text-[9px] text-white/40 font-mono">${row.tarikh}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="font-black text-cyan-400">⭐ ${row.skor} Pts</p>
                        <p class="text-[10px] text-emerald-400 font-mono font-bold">⏱️ ${paparanMasa}</p>
                    </div>
                </div>
            `;
        });
...
window.togglePasswordVisibility = function() {
    ...
    // Tukar jenis atribut input antara 'password' dan 'text'
    if (passwordInput.type === 'password') {
        passwordInput.type = 'text';
        // Paparkan ikon mata pangkah (tutup), sembunyikan mata biasa
        if (eyeIconOpen) eyeIconOpen.classList.add('hidden');
        if (eyeIconClose) eyeIconClose.classList.remove('hidden');
    } else {
        passwordInput.type = 'password';
        // Paparkan ikon mata biasa, sembunyikan mata pangkah
...
// Status bunyi (Default: Benarkan)
window.isSoundEnabled = true;

// Gunakan frekuensi Web Audio API (Penjana Bunyi Sintetik Elektronik)
// Ini menghasilkan kesan bunyi ala game retro 8-bit tanpa memerlukan sebarang fail MP3/OGG!
window.playSound = function(soundName) {
    ...
    try {
        ...
        // =========================================================================
        // KESAN BUNYI BARU UNTUK MINIGAME (FLIGHT MISSION)
        // =========================================================================
        else if (soundName === 'laser') {
            // Bunyi tembakan laser peiuw! peiuw!
            osc.type = 'sawtooth';
            osc.frequency.setValueAtTime(880, t);
            ...
            // Bunyi letupan meteor / pelanggaran (Sintesis bunyi putih "white noise" berderu)
            osc.type = 'sawtooth';
            ...
            // Bunyi naik ke atas gembira ting-ting-ting!
            osc.type = 'sine';
