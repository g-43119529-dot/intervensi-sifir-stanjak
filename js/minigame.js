...
// =========================================================================
// ANTI-CRASH AUDIO GUARD (PENGAMAN KESAN BUNYI GLOBAL)
// =========================================================================
...
window.playShoot = function() { safeAudioCall('audioShoot', 'Tembakan dilepaskan (tanpa bunyi).'); };
window.playExplosion = function() { safeAudioCall('audioExplosion', 'Musuh meletup (tanpa bunyi).'); };
window.playHit = function() { safeAudioCall('audioHit', 'Kapal dilanggar (tanpa bunyi).'); };
window.playCoin = function() { safeAudioCall('audioCoin', 'Syiling dipungut (tanpa bunyi).'); };
window.playPowerUp = function() { safeAudioCall('audioPowerUp', 'Power-up diaktifkan (tanpa bunyi).'); };
    ...
    if (!invincible) {
      enemies = enemies.filter(e => {
        if (e.x < player.x + player.w && e.x + e.w > player.x && e.y < player.y + player.h && e.y + e.h > player.y) {
          ...
          lives--; window.playHit(); invincible = true; invTime = 1.5;
